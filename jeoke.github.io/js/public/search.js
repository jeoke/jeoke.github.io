...
function CreateOb (src,description) {
      var arrob = new Object();
      arrob.src = src;
      arrob.description = description;
      return arrob;
}; 

var wrapd = document.getElementById("wrap");

var formtip = document.getElementsByClassName("form-group")[0];
...
function searchall(e) {
  ...
  for (var i = 0; i < bookdata.length; i++) {
      if (bookdata[i].description.match(pattern)) {
         bookob = CreateOb("book.html",bookdata[i].description);
         searchdata.push(bookob);
      }
    }

    for (var i= 0;i<agriculturedata.length;i++) {
      if (agriculturedata[i].name.match(pattern)) {
         agr = CreateOb("agriculture.html",agriculturedata[i].name);
         searchda.push(agr);
      }
    }

    for (var i = 0; i < computerdata.length; i++) {
      if (computerdata[i].name.match(pattern)) {
        compu = CreateOb("computersch.html",computerdata[i].name);
         searchc.push(compu);
      }
    }

    for (var i = 0; i < flowdata.length; i++) {
      if (flowdata[i].name.match(pattern)) {
         flowd = CreateOb("flow.html",flowdata[i].name);  
         searchf.push(flowd);
      }
    }

    for (var i = 0; i < knowledge.length; i++) {
      console.log(knowledge[i].name.match(pattern))
      if (knowledge[i].name.match(pattern) !== null) {
         knowl = CreateOb("knowledge.html",knowledge[i].name);
         searchi.push(knowl); 
      }
    }

function indexofNumber(arr,ele){

   var searchindex;
      for(i=0;i<arr.length;i++){
        if (arr[i].description === ele) {
          searchindex = 1;
        } else {
          continue;
        }
    ...
    function pushArray(arr,arr2){
      ...
      if (arr2.length !== 0) {
                  ...
                  if (arr.length !==0) {
                    for (var i = 0; i < arr.length; i++) {
                     if (arr[i].description === arr2[j].description) {
                         break;
                     } else {

                      var result = indexofNumber(arr,arr2[j].description);
                        if(result !== 1){
                          arr.push(arr2[j]);
                        }
      ...
      pushArray(searchdatall,searchdata);
      pushArray(searchdatall,searchda);
      pushArray(searchdatall,searchc);
      pushArray(searchdatall,searchf);
      pushArray(searchdatall,searchi);

      var nsearchlength;

      nsearchlength = searchdatall.length;

      if(nsearchlength >= searchlength){
            searchlength=nsearchlength;
          var formdiv = document.createElement("div");
          var wrapdiv = document.createElement("div");
          wrapdiv.setAttribute("id", "wrap");
        for (var i = 0; i < searchdatall.length; i++) {
          var formdiv = document.createElement("div");
          diva = document.createElement("a");
          diva.innerHTML = searchdatall[i].description;
          console.log(window.location.href);debugger;
          var  url=window.location.href;
          var location =url.substring(url.lastIndexOf('/')+1,url.length);
          var loca = location.toString();console.log(searchdatall[i].src);
          debugger;
          if (loca === "index.html") {
            diva.href = "./navlink/" + searchdatall[i].src;
          } else {
            diva.href = searchdatall[i].src;
          }
        
          formdiv.appendChild(diva);
          wrapdiv.appendChild(formdiv);  
      }  
      formtip.appendChild(wrapdiv);
     }  
